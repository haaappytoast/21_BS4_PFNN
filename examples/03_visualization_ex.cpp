...
public:
    ...
    void start() override
    {
        ...
        const auto &pose1 = motion.poses.at(f - 1);
        const auto &pose2 = motion.poses.at(f);
        // model->set_pose(pose, agl::anubis_motion_joint_names);
        model->set_pose(pose2);
        model->update_mesh();
        model->root()->update_world_trf_children(); // update global trfs

        Mat4 root = root_info.root_world_trf.at(f-1);
        vVec3 world_jnt_vel = framesData.at(f).jnts_local_velocity;
        // for(int i = min_frame; i < max_frame; ++i)
        // {
                // Mat4 root_trf = framesData.at(i).world_root_trf;

                // std::cout << "difference:\n"
                //             <<root_info.root_world_trf.at(i) - root_trf << std::endl;
        // }

        //* check local jnt_vel
        // for(int i = 0; i< world_jnt_vel.size(); ++i)
        // {
        //     // model->set_pose(pose, agl::anubis_motion_joint_names);
        //     model->set_pose(pose1);
        //     model->update_mesh();
        //     Vec4 world_vel = Vec4::Zero();
        //     world_vel.block<3, 1>(0, 0) = framesData.at(f).jnts_local_velocity.at(i);
        //     world_jnt_vel.at(i) = (root * world_vel).block<3, 1>(0, 0);
        //     std::string jnt = joint_names.at(i);
        //     Vec3 dpos = model->joint(jnt)->world_pos() + world_jnt_vel.at(i) * 1.0f/60.0f;

        //     // model->set_pose(pose, agl::anubis_motion_joint_names);
        //     model->set_pose(pose2);
        //     model->update_mesh();
        //     Vec3 real_pos = model->joint(jnt)->world_pos();
        //     std::cout << "difference:\n" 
        //               << dpos - real_pos << std::endl;
        // }

        //* check local root_ang_vel
        // for (int i = min_frame + 1; max_frame; ++i)
        // {
        //     float dTheta = framesData.at(i).root_angular_velocity;
        //     Mat rq = AAxis(dTheta, Vec3::UnitY()).toRotationMatrix();
        //     Mat3 root_gl_rot = framesData.at(i - 1).world_root_trf.block<3, 3>(0, 0) * rq;
        //     std::cout << "difference:\n"
        //               << root_gl_rot - framesData.at(i).world_root_trf.block<3, 3>(0, 0) << std::endl;
        // }


        // agl::Pose pose;
        // pose.root_position = model->joint("Hips")->local_pos();
        // pose.local_rotations.resize(model->joints().size(), Quat::Identity());
        // model->set_pose(pose);

        // for (auto jnt: model->joints())
        // {   std::cout << jnt->name() << std::endl;
        //     std::cout << jnt->local_rot_mat() << std::endl;
        //     std::cout << "****************" << std::endl;
        // }
    }
    ...
    {
        // camera_fix(root_info.root_world_trf.at(pidx), model, 1.5f * M_PI, Vec4(0, 1, 4, 1));
        ...
        // for (auto jnt : model->joints())
        // {
        //     if (jnt->name() == "Head" || jnt->name() == "Neck" ||jnt->name() ==  "LeftHand" ||jnt->name() ==  "RightHand")
        //     {
        //         std::cout << jnt->name() << "\n"
        //                   << jnt->local_rot_mat() << std::endl;
        //         std::cout << "***************\n" << std::endl;
        //     }
        // }
